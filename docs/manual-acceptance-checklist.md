...
- [x] QuickPanel 搜索输入非连续字符（如对"clipboard manager"输入"cbm"）能命中记录。
- [x] 搜索中文时按字符子序列命中（如对"剪贴板历史"输入"剪史"）。
- [x] 完整子串命中的记录排在纯模糊命中的记录之前。
- [x] 搜索激活时置顶记录仍显示在 Pinned 区且位于最上方。
- [x] QuickPanel 搜索命中时，列表行内命中字符以强调色+加粗显示；清空搜索后高亮消失。
- [x] 中文命中高亮逐字符正确（无错位、无半个字符高亮）。
...
- 补跑推迟说明 2026-07-02(以下三项暂缓,非遗漏):
  - `brew upgrade --cask clipboardapp` N→N+1 与"升级后辅助功能权限保持":需存在更高版本才能测,推迟到真正的 0.2.0 发布时一并验证(升级后 postflight 亦应再次去 quarantine)。
    - 补跑记录 2026-07-07:0.2.0 发布后执行 `brew update && brew upgrade --cask clipboardapp`,0.1.0→0.2.0 升级成功;`/Applications` 实装 CFBundleShortVersionString=0.2.0,签名 `Authority=ClipboardApp Local Code Signing`,`xattr` 无 quarantine(升级路径 postflight 生效);启动后 `Cmd+Shift+V` 呼出 QuickPanel 回车自动粘贴直接生效、无重新授权提示——辅助功能权限跨版本保持。结论: PASS。
...
- [x] 设置「通用 › 启动」开启"登录时自动启动"后，重启 macOS，ClipboardApp 自动出现在菜单栏（稳定自签名构建验证）。
- [x] 关闭"登录时自动启动"后，系统设置「登录项」中对应条目消失，重启后不再自动启动。
- [x] 系统设置中手动移除登录项后，设置页保持打开的状态下，开关在 1~2 秒内自动同步为关闭（无需重新打开设置页）。
- [x] `swift run` 场景（非 .app 包）下开关禁用并显示原因说明。

## 设置窗口 Dock/Cmd+Tab 可达性(动态 activation policy)

- [x] 打开设置窗口时 Dock 出现 ClipboardApp 图标，Cmd+Tab 列表中可见；切到其它 App 后可用 Cmd+Tab 或点 Dock 图标切回设置页。
- [x] 关闭设置窗口后 Dock 图标与 Cmd+Tab 条目消失，恢复纯菜单栏形态。
- [x] 设置窗口打开期间（policy 为 regular）：全局快捷键呼出 QuickPanel、选择粘贴、Esc 关闭等行为不变。
- [x] 设置窗口最小化后再从 Dock 恢复正常；恢复后关闭窗口仍能正确移除 Dock 图标。

- 验收记录 2026-07-07：Maccy daily parity 本轮新增条目（开机自启、模糊搜索与命中高亮、历史排序、菜单栏 Option+点击、粘贴后保留面板、设置窗口 Dock 可达性）由维护者在稳定自签名构建 `8f47811`（`Authority=ClipboardApp Local Code Signing`）上物理验收通过，含真实重启验证开机自启、系统设置外部移除登录项的实时同步、keep-open 跨 App（A→B）粘贴与 Terminal/浏览器地址栏自动粘贴可靠性。结论: PASS。

## 历史排序选项(HistorySortOrder)

- [x] 「历史」设置切到"复制次数"后，QuickPanel 空搜索浏览时高频记录排前。
- [x] 切回"最近复制"后恢复现状排序；置顶区顺序在三种排序下都不变。
- [x] 排序为"复制次数"时数字快捷键 Cmd+1~9 跟随新的可视顺序。

## 菜单栏 Option+点击快捷操作 + 暂停态图标

- [x] Option+点击菜单栏图标切换暂停/恢复采集，图标在暂停时变为 pause 形态。
- [x] Option+Shift+点击菜单栏图标后，下一次复制不入历史，再下一次恢复正常。
- [x] 从设置页切换"暂停采集"时菜单栏图标同步更新。
- [x] 暂停态与存储健康色（橙/红）可同时呈现，互不覆盖。

## 粘贴后保留面板(QuickPanel keepOpenAfterPaste)

- [x] 开启"粘贴后保留面板"后：自动粘贴模式下 Return 粘贴成功，目标 App 收到内容且面板保持可见。
- [x] 同设置下"仅复制"模式：选择后面板保持可见，手动 Cmd+V 粘贴成功。
- [x] 关闭该设置后恢复现状：粘贴后面板关闭。
- [x] 保留面板时 Esc 关闭面板并归还焦点。
- [x] （风险验证）自动粘贴 + 保留面板在 Terminal / 浏览器地址栏均能正确投递 Cmd+V；若不可靠，按 spec 降级为仅"仅复制"模式生效并在 spec 追记。
- [x] 保留面板开启时：先粘贴到 App A，再点击 App B 的输入框，回到面板选择另一条记录粘贴，内容落在 App B。
- [x] 保留面板开启时：粘贴完成后面板仍可见但目标 App 持有焦点；点击面板可恢复键盘操作（Return/方向键/数字键）。
